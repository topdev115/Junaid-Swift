...
public class PieChartView: UIView {
    ...
    /// inner circle color of the pie
    public var innerCircleColor: UIColor = .purple {
        didSet {
            setNeedsLayout()
        }
    }
    ...
    private var items: [PieChartItem] = [PieChartItem]()
    ...
    override public func draw(_ rect: CGRect) {
        ...
        // draw circle stroke.
        let outerStroke = UIBezierPath(arcCenter: center, radius: radius/2 + 1, startAngle: 0, endAngle: 2 * π, clockwise: true)
        
        outerStroke.lineWidth = 1
        strokeColor.setStroke()
        outerStroke.lineCapStyle = style
        outerStroke.stroke()
        
        let innerStroke = UIBezierPath(arcCenter: center, radius: radius/2 - arcWidth + 1, startAngle: 0, endAngle: 2 * π, clockwise: true)
        
        innerStroke.lineWidth = 1
        strokeColor.setStroke()
        innerStroke.lineCapStyle = style
        innerStroke.stroke()
        
        // draw inner circle.
        if background == nil {
            let r2 = radius/2 - arcWidth
            background = UIView(frame: CGRect(x: center.x - r2, y: center.y - r2, width: r2 * 2.0, height: r2 * 2.0))
            self.addSubview(background!)
            
            background!.layer.masksToBounds = true
            background!.layer.cornerRadius = background!.bounds.size.height / 2.0
        }
        background?.backgroundColor = innerCircleColor
    ...
    convenience init(items: [PieChartItem], centerTitle: String? = nil) {
        self.init()
        self.items = items
    ...
    private func drawCircle(){
        items.forEach { (item) in
            ...
            if(!isAnimationActivated) {
                // Draw circle path
                circlePath.lineWidth = arcWidth - 1
                circlePath.lineCapStyle = style
                ...
                shapeLayer.lineWidth = arcWidth - 1
            ...
            if(centerLabel == nil && centerTitle != nil) {
                ...
                centerLabel?.font = UIFont(name: "HelveticaNeue", size: 25.0)
    ...
    private func showChildTitles() {
        if (titlesView == nil) {
            ...
            items.forEach({ (item) in
                let view = ChartTitleView(item: item)
